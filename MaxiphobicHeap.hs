...
node :: (Ord a) => Heap a -> Heap a -> Heap a -> Heap a
node h@(Node x w lh rh) lh' rh'  
    | w  >= s1 && w  >= s2 = Node x s h (merge lh' rh')
    | s1 >= w  && s1 >= s2 = Node x s lh' (merge h rh')
    | s2 >= w  && s2 >= s1 = Node x s rh' (merge h lh')
    where
        s1 = size lh'
        s2 = size rh'
        s = w + s1 + s2
